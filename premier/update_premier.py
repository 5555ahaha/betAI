...
os.system('rm /tmp/outpremier.csv')
os.system('rm data/premier.csv')
stagioni={'0809', '0910', '1011', '1112', '1213', '1314', '1415', '1516', '1617', '1718', '1819'}
fout=open("data/premier.csv","w")
os.system('mkdir seasons')
for season in stagioni:
	os.system('wget -O seasons/'+season+'.csv http://www.football-data.co.uk/mmz4281/'+season+'/E0.csv')
	time.sleep(0.2)
# first file:
...
for season in stagioni:
    ...
    for line in f:
        ...
        fout.write(line)
fout.close()
data = csv.reader(open('data/premier.csv','r'))
header = data.next()
datafy = csv.writer(open('/tmp/outpremier.csv', 'w'))
for row in data:
	try:
...
datafy = csv.reader(open('/tmp/outpremier.csv', 'r'))
datafy = sorted(datafy, key = lambda row: datetime.strptime(row[1], "%d/%m/%Y"), reverse=False)
with open('data/premier.csv', 'w') as stream:
